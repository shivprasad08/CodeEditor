...
export async function runCode({ language, code, input = '', timeoutMs = 10000 }) {
  ...
  try {
    ...
    const mode = String(process.env.CODE_RUNNER_MODE || 'auto').toLowerCase();
    let execResult = null;
    let runner = 'docker';
    ...
    if (mode === 'local') {
      ...
      const preferLocalInAuto = String(process.env.CODE_RUNNER_AUTO_PREFER_LOCAL ?? 'true').toLowerCase() !== 'false';

      if (mode === 'auto' && preferLocalInAuto && languageConfig.supportsLocal) {
        execResult = await runWithLocalNode({
          tempDir,
          languageConfig,
          input,
